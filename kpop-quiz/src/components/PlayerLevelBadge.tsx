...
const PlayerLevelBadge: React.FC = () => {
  const { xp, userName } = useGameStore();
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl px-4 py-3 shadow-lg border-2 border-purple-200 flex items-center gap-3 max-w-sm mx-auto mb-6"
    >
      <div className={`bg-gradient-to-r ${LEVEL_COLORS[level]} rounded-xl w-12 h-12 flex items-center justify-center text-white font-fredoka font-bold text-xl shadow-md flex-shrink-0`}>
        {level}
      </div>
